...
enum_dirList = open('/usr/share/CafeScan/dicts/enum_dir.txt', "r").read().splitlines()
enum_userList = open('/usr/share/CafeScan/dicts/enum_user.txt', "r").read().splitlines()
enum_passList = open('/usr/share/CafeScan/dicts/enum_pass.txt', "r").read().splitlines()
enum_dirListBig = open('/usr/share/CafeScan/dicts/enum_dirBig.txt', "r").read().splitlines()
hostServices = {}
...
def printHelp():
	...
	print(f'	{bcolors.OKBLUE}-p, --ports {bcolors.ENDC}# Scan Specific Port(s) Seprator; ","')
	...
	print(f'	{bcolors.OKBLUE}--gbc, --grab-banners-cve {bcolors.ENDC}# Try To Grab Service Banners & Look for Possible CVE(s)')
	print(f'	{bcolors.OKBLUE}--as, --scan-aggressive {bcolors.ENDC}# Aggressive Scan')
	print(f'	{bcolors.OKBLUE}--ws, --scan-webdir {bcolors.ENDC}# Web Directories/Paths Scan')
	sys.exit()
...
def main(argv):
	...
	print('CafeScan v0.2 ( https://github.com/Gomez0015/CafeScan )\n')

	try:
		# Define CLI arguments
		opts, args = getopt.getopt(argv,"hi:p:at",["help", "ports=","ip-address=","all-ports","top-ports", "grab-banners", "gb", "grab-banners-cve", "gbc", "as", "scan-webdir", "ws"])
	...
	for opt, arg in opts:
		if opt in ("-h", "--help"):
			...
			portArray = arg.split(',')
			ports = [int(x) for x in portArray]
		...
		elif opt in ("--gbc", "--grab-banners-cve"):
			global cveScan
			grabBanners = True
			cveScan = True
		...
		elif opt in ("--as", "--scan-aggressive"):
			global aggressiveScan
			aggressiveScan = True

		elif opt in ("--ws", "--scan-webdir"):
			global webDirScan
			webDirScan = True
...
def scanWebDir(protocol, port):
	...
	try:
		futures=[session.get(f'{protocol}://{target}:{port}/{dir}', allow_redirects=False) for dir in enum_dirListBig]
	except:
		print(f'-	An Error Occured, Stopping Enumeration')
	...
	for future in as_completed(futures):
		...
		if resp.status_code == 200 or resp.status_code == 301:
			resp.request.url = resp.request.url.replace(f'{protocol}://{target}:{port}/', '')
				...
				try:
					...
					for payload in payloads:
						...
						if(hostServices.get(port, 'none') == 'HTTP' or hostServices.get(port, 'none') == 'HTTPS'):
							...
							if len(http_title) > 0:
								http_title[0] = http_title[0].strip().replace("<title>", "").replace("</title>", "")
								print(f'-	Title: {http_title[0]}')

								if cveScan == True:
									cveResults = nvdlib.searchCVE(keyword=http_title[0].replace('/', ' '))
									detectedCVEList = f'-	CVE(s) Detected: '

									for cve in cveResults:
										detectedCVEList += f'{bcolors.HEADER + cve.id + bcolors.ENDC}, '

									if(len(cveResults) > 0):
										print(detectedCVEList)
							...
							if cveScan == True:
								cveResults = nvdlib.searchCVE(keyword=os.linesep.join(response.split(os.linesep)[:1]))
								if(len(cveResults) > 0):
									print(f'-	CVE Detected: {bcolors.HEADER + cveResults[0].id + bcolors.ENDC}')
...
def smbGrabBanner():
	...
	if smbClient.connect(target, 139):
		for share in smbClient.listShares():
			if(share.name == "IPC$"):
				print(f"-	{share.comments.replace('IPC Service ', '').replace('(', '').replace(')', '')}")

				if cveScan == True:
					cveResults = nvdlib.searchCVE(keyword=share.comments.replace('IPC Service ', '').replace('(', '').replace(')', '').replace('-', ' '))
					if(len(cveResults) > 0):
						print(f'-	CVE Detected: {bcolors.HEADER + cveResults[0].id + bcolors.ENDC}')
...
if __name__ == "__main__":
	...
	try:
		...
		print('CafeScan finished in: ' + str(datetime.now() - startTime))